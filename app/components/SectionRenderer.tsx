      ...
      {list.map((s: any, i: number) => {
        switch (s._type) {
          case 'hero': {
            ...
            return (
              <section key={i} className={`relative overflow-hidden ${hasImage ? '' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50'}`}>
                <div className={hasImage ? 'relative h-[56vh] md:h-[68vh]' : 'py-20'}>
                  {hasImage && (
                    <div className="absolute inset-0">
                      <Image src={s.image.asset.url} alt="" fill priority className="object-cover" />
                  ...
                  <div className={`relative z-10 max-w-7xl mx-auto ${hasImage ? 'h-full' : ''} px-4 sm:px-6 flex items-center`}>
                    <div className={`max-w-3xl ${textColor}`}>
                      ...
                      {s.subhead && <p className={`mt-4 text-lg md:text-xl ${subColor}`}>{s.subhead}</p>}
                      {s.cta?.href && (
